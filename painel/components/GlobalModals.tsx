...
export const GlobalModals = ({
    ...
    const [showDriverList, setShowDriverList] = React.useState(false);
            ...
            <div className={`${theme.card} w-full max-h-[85dvh] md:max-h-[90vh] md:max-w-xl rounded-t-3xl md:rounded-2xl border ${theme.border} shadow-2xl flex flex-col page-transition`}>
                ...
                <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-5 pb-10">
                    ...
                    {modal === 'passenger' && (<>
                        ...
                        <div className="flex flex-col gap-1.5"><label className="text-xs font-bold opacity-60 ml-1">Bairro</label><select id="input-passenger-neighborhood" className="bg-black/10 border border-white/10 text-white rounded-xl px-4 py-3.5 h-14" value={formData.neighborhood || ''} onChange={(e:any)=>setFormData((prev:any) => ({...prev, neighborhood:e.target.value}))}><option value="" disabled>Selecione um bairro</option>{bairrosList.map(b=><option key={b} value={b} className="bg-slate-900">{b}</option>)}</select></div>
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        ...
                                        <button 
                                            ...
                                            className="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 h-14 flex items-center justify-between hover:bg-white/5 transition-all"
                                        >
                                            <div className="flex flex-col items-start truncate">
                                                {formData.driverId ? (
                                                    ...
                                                    <span className="text-sm font-bold text-white/50">Selecione um motorista</span>
                                                )}
                                            </div>
                                            <Icons.ChevronDown size={18} className={`opacity-40 transition-transform ${showDriverList ? 'rotate-180' : ''}`} />
                                        </button>

                                        {showDriverList && (
                                            <div className={`mt-2 ${theme.card} rounded-xl p-3 shadow-2xl space-y-3 animate-in fade-in slide-in-from-top-2 duration-200 z-50`}>
                                                <div className="flex items-center gap-2 bg-black/40 px-3 py-2 rounded-lg border border-white/10 focus-within:border-white/30 transition-all">
                                                    <Icons.Search size={14} className="opacity-40" />
                                                    <input 
                                                        type="text" 
                                                        placeholder="Procurar motorista..." 
                                                        className="bg-transparent border-none outline-none text-xs w-full placeholder:opacity-30"
                                                        value={driverSearch}
                                                        onChange={(e) => setDriverSearch(e.target.value)}
                                                    />
                                                </div>
                                                <div id="driver-grid-selection" className="flex flex-col gap-1 max-h-60 overflow-y-auto pr-1 custom-scrollbar">
                                                    {data.drivers.filter((d:any) => { 
                                                        ...
                                                        return (
                                                            <div 
                                                                ...
                                                                className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-all border ${isSelected ? `bg-${themeKey}-500/20 border-${themeKey}-500 ring-1 ring-${themeKey}-500` : `${theme.inner} hover:bg-white/5`}`}
                                                            >
                                                                ...
                                                                {isSelected && <Icons.Check size={14} className={`text-${themeKey}-500`} />}
                                                            </div>
                                                        );
                                                    })}
                                                </div>
                                ...
                                <div className="flex flex-col h-full">
                                    <div className="bg-black/10 p-5 rounded-xl border border-white/10 flex-1 flex flex-col">
                                        ...
                                        <div className="flex flex-col gap-1.5 mb-4">
                                            ...
                                            >
                                                <div className="flex flex-col items-start truncate">
                                                    {suggestedTrip.driver?.id ? (
                                                        <>
                                                            <span className="text-sm font-bold text-white">
                                                                {suggestedTrip.driver.name}
                                                            </span>
                                                            <span className="text-[10px] opacity-50 font-bold">
                                                                {(() => {
