...
const AppContent = () => {
  const { user, isAuthenticated, isLoading, logout, updateActivity } =
    useAuth();
  ...
  const [view, setView] = useState(
    () => localStorage.getItem("nexflow_active_view") || "dashboard",
  );
  ...
  const [tableTab, setTableTab] = useState(
    () => localStorage.getItem("nexflow_table_tab") || "geral",
  );
  ...
  const [mipDayType, setMipDayType] = useState(() =>
    new Date().getDate() % 2 !== 0 ? "odd" : "even",
  );
  ...
  const [soundEnabled, setSoundEnabled] = useState(
    () => localStorage.getItem("nexflow_sound_enabled") !== "false",
  );
  const [popupsEnabled, setPopupsEnabled] = useState(
    () => localStorage.getItem("nexflow_popups_enabled") !== "false",
  );
  const [siteNotificationsEnabled, setSiteNotificationsEnabled] = useState(
    () => localStorage.getItem("nexflow_site_notifs_enabled") !== "false",
  );
  ...
  useEffect(() => {
    if (isOnline && isDbConnected && pendingOps.length > 0) {
      const syncOps = async () => {
        ...
        notify(
          `Sincronizando ${opsToProcess.length} alterações pendentes...`,
          "info",
        );
  ...
  const trackApiMetric = useCallback(
    (type: "aiStudio" | "magicRegistration", usdCost: number) => {
      if (!db) return;
      const now = new Date();
      ...
      db.ref(`api_metrics_v3/${monthKey}`).transaction((curr: any) => {
        ...
        if (type === "aiStudio") {
          metrics.totalAiStudioCostBrl =
            (metrics.totalAiStudioCostBrl || 0) + brlCost;
        } else {
          metrics.totalMagicRegistrationCostBrl =
            (metrics.totalMagicRegistrationCostBrl || 0) + brlCost;
  ...
  const [isSystemSelectorExpanded, setIsSystemSelectorExpanded] =
    useState(true);
  const [subData, setSubData] = useState<any>(null);
  const [systemContext, setSystemContext] = useState("Pg");
  const [targetAppointmentDate, setTargetAppointmentDate] = useState<
    string | null
  >(null);

  const getFolgasForDate = useCallback(
    (targetDateStr: string) => {
      const tableSystemContext =
        user?.username === "Breno" && systemContext === "Mistura"
          ? "Pg"
          : systemContext;
      ...
      if (saturdayRotation && !saturdayFolgaDisabled) {
        ...
        if (cycleStartIdx !== -1) {
          ...
          const normalizedIdx =
            idx < 0
              ? (idx + SATURDAY_CYCLE.length) % SATURDAY_CYCLE.length
              : idx;
          ...
          if (currentSaturdayWeekday) {
            ...
            result["SÁBADO"] = Array.isArray(weekdayFolgas)
              ? [...weekdayFolgas]
              : [];
          }
        }
  ...
  const billingData = useMemo(() => {
    ...
    const filtered = trips.filter((t: any) => {
      ...
      if (user && user.role === "operador") {
        ...
        const isPaidByMe =
          t.paymentStatus === "Pago" && t.receivedBy === user.username;
    ...
    filtered.forEach((t: any) => {
      ...
      if (!groups[t.date])
        groups[t.date] = { date: t.date, totalValue: 0, trips: [] };
    ...
    return {
      ...
      groups: Object.values(groups).sort((a: any, b: any) =>
  ...
  const [adminNotifications, setAdminNotifications] = useState<
    { id: string; message: string; realId: string }[]
  >([]);
  ...
  const [orderedMenuItems, setOrderedMenuItems] = useState(() => {
    ...
    if (saved) {
      try {
        ...
        return [...DEFAULT_MENU_ITEMS].sort(
  ...
  const TOUR_STEPS = [
    {
      ...
      content:
        "O sistema mais completo para gestão de lotação e fretamento. Vamos te mostrar como organizar sua rotina, economizar tempo e profissionalizar seu atendimento em poucos minutos!",
      target: null,
    },
    {
      title: "Menu Inteligente & Flexível",
      content:
        "Aqui você acessa todas as ferramentas. DICA: Você pode segurar e arrastar os ícones para reorganizar o menu conforme sua preferência de uso diário!",
    ...
    },
    {
      ...
      content:
        "Acompanhe o crescimento do seu negócio com estatísticas em tempo real. Veja quantos passageiros estão ativos, o total de viagens realizadas e até uma estimativa do seu faturamento diário!",
    ...
    },
    {
      ...
      content:
        "A tecnologia a seu favor! Não perca tempo preenchendo formulários longos. Basta digitar ou ditar a mensagem do cliente e nossa IA identifica nome, bairro, horário e cria o agendamento sozinha.",
    ...
    },
    {
      ...
      content:
        "Gerencie quem viaja com você todos os dias. Aqui ficam os passageiros com horários definidos que ainda não foram alocados em uma viagem específica.",
    ...
    },
    {
      ...
      content:
        "Seu cadastro completo de clientes. Pesquise por nome ou bairro, ligue diretamente pelo WhatsApp e veja o histórico de cada um de forma organizada.",
    ...
    },
    {
      ...
      content:
        "O coração da operação! Controle a fila de motoristas, quem já baixou, quem está confirmado e quem está na espera (Lousa). Tudo atualiza instantaneamente para todos os usuários.",
    ...
    },
    {
      ...
      content:
        "Alterne rapidamente entre a Tabela Geral, Confirmados, Lousa e Madrugada. Cada aba tem uma função específica para organizar o fluxo de saída das vans.",
      target: "#table-tabs",
      placement: "bottom",
    },
    {
      ...
      content:
        "Crie viagens, aloque motoristas e envie a lista de passageiros formatada diretamente para o WhatsApp do motorista com apenas um clique!",
    ...
    },
    {
      ...
      content:
        "Nada se perde! Acesse o histórico de meses anteriores e gere relatórios automáticos em TXT para conferência de faturamento e produtividade.",
    ...
    },
    {
      ...
      content:
        "Mantenha o caixa em dia. O sistema identifica quem já pagou e quem está pendente. Você pode cobrar os inadimplentes no WhatsApp de forma profissional e rápida.",
    ...
    },
    {
      ...
      content:
        "Um diferencial no seu atendimento! Registre itens esquecidos nas vans e localize os donos rapidamente, evitando confusões e reclamações.",
    ...
    },
    {
      ...
      content:
        "Controle a escala dos motoristas. Marque folgas programadas ou aplique 'ganchos' (suspensões temporárias) para manter a ordem na fila da lousa digital.",
    ...
    },
    {
      ...
      content:
        "Aqui você gerencia sua conta, altera sua senha, personaliza as cores do sistema e acessa as configurações avançadas de faturamento e permissões.",
    ...
    },
    {
      title: "Tudo Pronto! 🚀",
      content:
        "Você agora conhece o básico do Bora de Van. Explore cada tela, faça testes e veja como sua operação ficará muito mais organizada. Sucesso na rodagem!",
      target: null,
    },
  ];
  ...
  const addPersistentNotification = (msg: string) => {
    setPersistentNotifications((prev) => [
      ...prev,
      { id: Date.now().toString(), message: msg },
    ]);
  };
  ...
  const removePersistentNotification = (id: string) => {
    if (id.startsWith("admin_notif_")) {
      ...
      if (user?.username) {
        db.ref(
          `admin_notifications/${realId}/dismissedBy/${user.username}`,
        ).set(true);
      }
    } else {
  ...
  const dismissAllAdminNotifications = () => {
    ...
    adminNotifications.forEach((n) => {
      db.ref(
        `admin_notifications/${n.realId}/dismissedBy/${user.username}`,
      ).set(true);
    });
  };
  ...
  const logAction = useCallback(
    async (action: string, details: string) => {
      if (!user || user.username === "Breno" || user.isImpersonated || !db)
        return;
  ...
  const dbOp = async (
    ...
    if (!isSync && (!isOnline || !isDbConnected)) {
      ...
      notify(
        "Modo Offline: Alteração salva em cache e será sincronizada ao voltar a conexão.",
        "warning",
      );
      return;
    }
    ...
    if (
      targetSystem === "Mistura" &&
      ["passengers", "drivers", "trips", "lostFound"].includes(node)
    ) {
      // Se estiver no modo Mistura, tenta inferir o sistema do payload ou usa 'Pg' como fallback para o Breno
      targetSystem =
        payload &&
        payload.system &&
        ["Pg", "Mip", "Sv"].includes(payload.system)
          ? payload.system
          : "Pg";
    }
    ...
    const getPath = (system: string, nodeName: string) => {
      const tableSystemContext =
        user.username === "Breno" && system === "Mistura" ? "Pg" : system;

      if (["passengers", "drivers", "trips", "lostFound"].includes(nodeName)) {
        return system === "Pg" ? nodeName : `${system}/${nodeName}`;
      }
      if (nodeName === "lousa_order") {
        return tableSystemContext === "Pg"
          ? `daily_tables/${lousaDate}/lousaOrder`
          : `${tableSystemContext}/daily_tables/${lousaDate}/lousaOrder`;
      }
      if (nodeName === "madrugada_order") {
        return tableSystemContext === "Pg"
          ? "madrugada_config/list"
          : `${tableSystemContext}/madrugada_config/list`;
      }
      if (
        nodeName === "drivers_table_list" ||
        nodeName === "table_status" ||
        nodeName === "madrugada_config/list" ||
        nodeName.startsWith("daily_tables") ||
        nodeName.startsWith("system_settings")
      ) {
    ...
    try {
      ...
      if (type === "create") {
        ...
        const nextId =
          node === "passengers" || node === "drivers" || node === "trips"
            ? getNextId(node)
            : targetId || Date.now().toString();
        const finalId =
          targetId &&
          (node === "trips" || node === "newsletter" || node === "users")
            ? targetId
            : nextId;
        ...
        // Logging
        if (node === "passengers")
          logAction("Criou Passageiro", `Nome: ${finalPayload.name}`);
        else if (node === "drivers")
          logAction("Criou Motorista", `Nome: ${finalPayload.name}`);
        else if (node === "trips")
          logAction(
            "Criou Viagem",
            `Motorista: ${finalPayload.driverName} - ${finalPayload.time}`,
          );
        else if (node === "lostFound")
          logAction("Criou Achados e Perdidos", `Item: ${finalPayload.item}`);
        else if (node === "users")
          logAction("Criou Usuário", `Username: ${finalPayload.username}`);
        else if (node === "notes")
          logAction(
            "Criou Nota",
            `Texto: ${finalPayload.text?.substring(0, 30)}...`,
          );
        ...
        if (targetId) {
          if (payload && typeof payload === "object") {
            ...
            await ref.child(targetId).set(payload);
          }
        } else {
          // Para listas completas (como drivers_table_list), usamos set para evitar merge de índices
          if (
            node === "drivers_table_list" ||
            node === "madrugada_config/list" ||
            node === "canned_messages_config/list" ||
            node === "lousa_order" ||
            node === "madrugada_order"
          ) {
        ...
        // Logging Updates
        if (node === "lousa_order")
          logAction("Reorganizou Lousa", `Data: ${lousaDate}`);
        else if (node === "madrugada_order")
          logAction("Reorganizou Madrugada", `Data: ${currentOpDate}`);
        else if (node === "canned_messages_config/list")
          logAction("Editou Mensagens Prontas", "Lista atualizada");
        else if (node.startsWith("system_settings"))
          logAction("Editou Configurações", `Configuração: ${node}`);
        else if (node.startsWith("user_data"))
          logAction("Gerenciou Assinatura", `Nó: ${node}`);
        else if (node === "blocked_devices")
          logAction("Bloqueou Dispositivo", `ID: ${targetId}`);
        else if (node === "passengers") {
          const pName =
            payload.name ||
            data.passengers.find((p: any) => p.id === targetId)?.name ||
            "ID " + targetId;
          if (payload.status === "Bloqueado")
            logAction("Bloqueou Passageiro", `Nome: ${pName}`);
          else if (payload.status === "Ativo" && payload.blockReason === null)
            logAction("Desbloqueou Passageiro", `Nome: ${pName}`);
          else if (payload.time && !payload.name)
            logAction(
              "Reagendou Passageiro",
              `Nome: ${pName} para ${payload.date || "mesmo dia"} às ${payload.time}`,
            );
          else if (payload.name)
            logAction("Editou Passageiro", `Nome: ${payload.name}`);
        } else if (node === "drivers" && payload.name)
          logAction("Editou Motorista", `Nome: ${payload.name}`);
        else if (node === "drivers_table_list")
          logAction(
            "Editou Tabela Motoristas",
            "Lista de motoristas na tabela atualizada",
          );
        else if (node === "table_status")
          logAction("Alterou Status Tabela", "Status da tabela atualizado");
        else if (node === "notes")
          logAction(
            "Editou Nota",
            `Status: ${payload.completed ? "Concluída" : "Pendente"}`,
          );
        else if (node === "trips") {
          if (payload.paymentStatus === "Pago")
            logAction(
              "Marcou como Pago",
              `Viagem: ${payload.driverName || "ID " + targetId}`,
            );
          else if (payload.paymentStatus === "Pendente")
            logAction(
              "Removeu Pagamento",
              `Viagem: ${payload.driverName || "ID " + targetId}`,
            );
            ...
            logAction(
              "Alterou Status Viagem",
              `Viagem: ${payload.driverName || "ID " + targetId} -> ${payload.status}`,
            );
          else
            logAction(
              "Editou Viagem",
              `Motorista: ${payload.driverName || "ID " + targetId}`,
            );
        } else if (node === "lostFound")
          logAction(
            "Editou Achados e Perdidos",
            `Item: ${payload.item || "ID " + targetId}`,
          );
        else if (node === "users")
          logAction(
            "Editou Usuário",
            `Username: ${payload.username || "ID " + targetId}`,
          );
        ...
        if (
          ...
          if (isAdminAuthorized) {
            ...
            setAlertState({
              ...
              message:
                "Comportamento estranho detectado os dados excluídos serão recuperados, para apagar diversos dados do sistema logue com uma conta admin",
              type: "danger",
            });
        ...
        if (itemData) {
          ...
          if (
            ...
            setDeletedItemsBuffer((prev) => [
              ...prev,
              { system: deleteSystem, node, id: deleteId, data: itemData },
            ]);
          ...
          triggerUndo(
            undoAction,
            `Excluiu ${node === "passengers" ? "Passageiro" : node === "drivers" ? "Motorista" : "Item"}`,
          );
        }
        ...
        // Logging Deletions
        if (node === "passengers")
          logAction("Excluiu Passageiro", `ID: ${idToDelete}`);
        else if (node === "drivers")
          logAction("Excluiu Motorista", `ID: ${idToDelete}`);
        else if (node === "trips")
          logAction("Excluiu Viagem", `ID: ${idToDelete}`);
        else if (node === "lostFound")
          logAction("Excluiu Achados e Perdidos", `ID: ${idToDelete}`);
        else if (node === "users")
          logAction("Excluiu Usuário", `ID: ${idToDelete}`);
        else if (node === "notes")
          logAction("Excluiu Nota", `ID: ${idToDelete}`);
        else if (node === "blocked_devices")
          logAction("Desbloqueou Dispositivo", `ID: ${idToDelete}`);
  ...
  const togglePranchetaPayment = async (vaga: string) => {
    ...
    const tableSystemContext =
      user.username === "Breno" && systemContext === "Mistura"
        ...
        : systemContext;
    const path =
      tableSystemContext === "Pg"
        ? `prancheta/${viewedWeekId}/${vaga}`
        : `${tableSystemContext}/prancheta/${viewedWeekId}/${vaga}`;
    const ref = db.ref(path);
    ...
    if (current && current.paid) {
      ...
      if (user.role !== "admin" && current.receivedBy !== user.username) {
        return notify(
          "Apenas quem recebeu ou um ADMIN pode desmarcar este pagamento.",
          "error",
        );
      }
      await ref.remove();
      logAction(
        "Removeu Pagamento Prancheta",
        `Vaga: ${vaga} - Semana: ${viewedWeekId}`,
      );
  ...
  useEffect(() => {
    ...
    if (day === 3) {
      const lastNotif = localStorage.getItem(
        `notif_prancheta_wed_${currentWeekId}`,
      );
      if (lastNotif !== "true") {
        showAlert(
          "Dia de Cobrança! 💰",
          "Hoje é quarta-feira, dia de coletar as pranchetas no sistema PG.",
          "info",
        );
    ...
    if (day === 5) {
      const lastNotif = localStorage.getItem(
        `notif_prancheta_fri_${currentWeekId}`,
      );
      if (lastNotif !== "true") {
        showAlert(
          "Último Dia de Pagamento! ⚠️",
          "Hoje é sexta-feira, o último dia para pagar as pranchetas sem ficar riscado na tabela.",
          "warning",
        );
  ...
  const [activeSiteNotification, setActiveSiteNotification] =
    useState<any>(null);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      try {
        ...
        const pendingReminders = notesRef.current.filter(
          (n: any) =>
        ...
        if (pendingReminders.length > 0) {
          ...
          if (reminderAudioRef.current) {
            ...
            if (playPromise !== undefined) {
              playPromise.catch((e) => {
                console.warn(
                  "Audio playback blocked by browser. User interaction needed.",
                  e,
                );
              });
            }
  ...
  useEffect(() => {
    ...
    const handleNewNotif = (snap: any) => {
      ...
      // Filter by system
      const shouldShow =
        systemContext === "Mistura" || notif.system === systemContext;
      ...
      if (siteNotificationAudioRef.current) {
        ...
        siteNotificationAudioRef.current
          .play()
  ...
  useEffect(() => {
    ...
    const handleAdminNotifs = (snap: any) => {
      ...
      const activeNotifs = Object.entries(notifs)
        .map(([id, data]: any) => ({
          id: `admin_notif_${id}`,
          realId: id,
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (siteNotificationAudioRef.current) {
        ...
        siteNotificationAudioRef.current
          .play()
          .catch((e) => console.warn("Repetitive sound blocked:", e));
      }
    }, 3000);
  ...
  const changeTheme = (t: string) => {
    setThemeKey(t);
    if (user) {
      dbOp("update", "preferences", { theme: t });
      localStorage.setItem(`${user.username}_nexflow_theme`, t);
    }
  };
  ...
  const commandActions = useMemo(
    () => [
      ...
      },
      {
        ...
        action: () => {
          const now = new Date();
          const timeToUse = now.toLocaleTimeString("pt-BR", {
            hour: "2-digit",
            minute: "2-digit",
            timeZone: "America/Sao_Paulo",
          });
          setFormData({
            neighborhood: systemContext === "Mip" ? BAIRROS_MIP[0] : BAIRROS[0],
            status: "Ativo",
            payment: "Dinheiro",
            passengerCount: 1,
            luggageCount: 0,
      ...
      {
        label: "Tabela de Motoristas",
        icon: <Icons.List size={18} />,
        action: () => setView("table"),
      },
      {
        label: "Financeiro",
        icon: <Icons.Dollar size={18} />,
        action: () => setView("billing"),
      },
      {
        label: "Cadastro Mágico (IA)",
        icon: <Icons.Stars size={18} />,
        action: () => setAiModal(true),
        color: "purple-400",
      },
      {
        label: "Mudar Tema: Padrão",
        icon: <Icons.Settings size={18} />,
        action: () => changeTheme("default"),
      },
      {
        label: "Mudar Tema: Escuro",
  ...
  useEffect(() => {
    ...
    if (isMobile) {
      const isMenuTarget =
        step.target &&
        (step.target.includes("sidebar") || step.target.includes("menu-btn"));
  ...
  useEffect(() => {
    if (data.newsletter && data.newsletter.length > 0 && user) {
      ...
      const sortedNews = [...data.newsletter].sort(
      ...
      const visibleNews = sortedNews.filter((news: any) => {
        ...
        if (user.username === "Breno" || systemContext === "Mistura")
          return true;
  ...
  useEffect(() => {
    ...
    const path =
      systemContext === "Pg"
        ? "canned_messages_config/list"
        : `${systemContext}/canned_messages_config/list`;
    ...
    const pricePath =
      systemContext === "Pg"
        ? "system_settings/pricePerPassenger"
        : `${systemContext}/system_settings/pricePerPassenger`;
    ...
    // Extra Charge Value Listener
    const extraPricePath =
      systemContext === "Pg"
        ? "system_settings/extraChargeValue"
        : `${systemContext}/system_settings/extraChargeValue`;
    const extraPriceRef = db.ref(extraPricePath);
    const extraPriceCb = extraPriceRef.on("value", (snap: any) => {
      const val = snap.val();
      if (val) setExtraChargeValue(Number(val));
      else setExtraChargeValue(0);
    });
    ...
    let driversNode =
      tableSystemContext === "Pg"
        ? "drivers_table_list"
        : `${tableSystemContext}/drivers_table_list`;
    ...
    const driversCb = driversRef.on("value", (snap: any) => {
      ...
      if (val) {
        ...
        if (
          (tableSystemContext === "Mip" || tableSystemContext === "Sv") &&
          Array.isArray(val) &&
          val.length === INITIAL_SP_LIST.length &&
          ...
          setSpList(
            list.map((d: any) => ({ ...d, id: d.id || d.uid || d.vaga })),
          );
        }
      } else {
        setSpList([]);
    ...
    const rotDateRef = db.ref(
      tableSystemContext === "Pg"
        ? "system_settings/rotation_base_date"
        : `${tableSystemContext}/system_settings/rotation_base_date`,
    );
    ...
    const dailyRef = db.ref(
      tableSystemContext === "Pg"
        ? `daily_tables/${currentOpDate}`
        : `${tableSystemContext}/daily_tables/${currentOpDate}`,
    );
    ...
    const savedMenuRef = db.ref(
      `user_data/${user.username}/preferences/menuOrder`,
    );
    const savedMenuCb = savedMenuRef.on("value", (snap: any) => {
      ...
      if (savedOrder && Array.isArray(savedOrder)) {
        ...
        const reordered = uniqueSavedOrder
          .map((id) => DEFAULT_MENU_ITEMS.find((i) => i.id === id))
          .filter(Boolean);
        const missing = DEFAULT_MENU_ITEMS.filter(
          (i) => !uniqueSavedOrder.includes(i.id),
        );
    ...
    const swapsPath =
      tableSystemContext === "Pg"
        ? `folgas_swaps/${tableWeekId}`
        : `${tableSystemContext}/folgas_swaps/${tableWeekId}`;
    ...
    const folgasDisabledPath =
      tableSystemContext === "Pg"
        ? "system_settings/folgas_disabled"
        : `${tableSystemContext}/system_settings/folgas_disabled`;
    const satFolgaDisabledPath =
      tableSystemContext === "Pg"
        ? "system_settings/saturday_folga_disabled"
        : `${tableSystemContext}/system_settings/saturday_folga_disabled`;
    const customFolgasPath =
      tableSystemContext === "Pg"
        ? "system_settings/custom_default_folgas"
        : `${tableSystemContext}/system_settings/custom_default_folgas`;
    ...
    const saturdayFolgaDisabledCb = saturdayFolgaDisabledRef.on(
      "value",
      (snap: any) => {
        setSaturdayFolgaDisabled(!!snap.val());
      },
    );
    const customDefaultFolgasCb = customDefaultFolgasRef.on(
      "value",
      (snap: any) => {
    ...
    const currentPranchetaRef = db.ref(
      tableSystemContext === "Pg"
        ? `prancheta/${currentWeekId}`
        : `${tableSystemContext}/prancheta/${currentWeekId}`,
    );
    const currentPranchetaCb = currentPranchetaRef.on("value", (snap: any) => {
      setCurrentPranchetaData(snap.val() || {});
    });

    const duePranchetaRef = db.ref(
      tableSystemContext === "Pg"
        ? `prancheta/${dueWeekId}`
        : `${tableSystemContext}/prancheta/${dueWeekId}`,
    );
    const duePranchetaCb = duePranchetaRef.on("value", (snap: any) => {
      setDuePranchetaData(snap.val() || {});
    });

    const viewedPranchetaRef = db.ref(
      tableSystemContext === "Pg"
        ? `prancheta/${viewedWeekId}`
        : `${tableSystemContext}/prancheta/${viewedWeekId}`,
    );
    ...
    return () => {
      ...
      extraPriceRef.off("value", extraPriceCb);
  ...
  useEffect(() => {
    ...
    const coreNodes = [
      ...
      "lostFound",
      "prancheta",
    ]; // Added lostFound and prancheta here
    ...
    const setupListeners = async () => {
      ...
      if (systemContext === "Mistura") {
        ...
        const promises = systems.flatMap((sys) =>
          coreNodes.map((node) => fetchData(sys, node)),
        );
        ...
        coreNodes.forEach((node) => {
          const path =
            systemContext === "Pg" ? node : `${systemContext}/${node}`;
          const ref = db.ref(path);
          unsubs.push(attachChildListeners(ref, node));
        });
      }
  ...
  useEffect(() => {
    ...
    const checkAndClearMipTables = async () => {
      ...
      if (lastCleared && realOpDate > lastCleared) {
        ...
        console.log(
          `Mip tables cleared for new operational date: ${realOpDate}`,
        );
      } else if (!lastCleared) {
        await lastClearedRef.set(realOpDate);
      }
  ...
  const getRotatedList = (dateStr: string) => {
    ...
    const rotatedOriginals = [
      ...originalList.slice(mod),
      ...originalList.slice(0, mod),
    ];
  ...
  const getTableTimes = () => {
    ...
    list.forEach((driver: any) => {
      if (tableStatus[driver.vaga] === "confirmed") {
        const time = new Date(
          lastConfirmTime.getTime() + confirmCount * 30 * 60000,
        );
        confirmedTimes[driver.vaga] = time.toLocaleTimeString("pt-BR", {
          hour: "2-digit",
          minute: "2-digit",
          hour12: false,
        });
    ...
    const manageTempTrips = () => {
      ...
      lousaOrder.forEach((item: any) => {
        ...
        if (!item.riscado && !item.isNull) {
          const t = new Date(
            startLousaTime.getTime() + lousaIndex * 30 * 60000,
          );
          const timeStr = t.toLocaleTimeString("pt-BR", {
            hour: "2-digit",
            minute: "2-digit",
            hour12: false,
          });
      ...
      const existingIds = new Set(
        data.trips
          .map((t: any) => parseInt(t.realId || t.id))
          .filter((n: number) => !isNaN(n)),
      );
      ...
      activeSlots.forEach((slot: any) => {
        ...
        if (diffMinutes >= 0 && diffMinutes <= 45) {
          ...
          const driverDb = driverSp
            ? data.drivers.find(
                (d: any) =>
                  d?.name?.toLowerCase() === driverSp?.name?.toLowerCase(),
              )
            : null;
          ...
          const exists = data.trips.some(
            (t: any) =>
              (t.driverId === cleanDriverId ||
                (t.driverName &&
                  t.driverName.toLowerCase().trim() ===
                    driverDb.name.toLowerCase().trim())) &&
          ...
          const pendingExists = Object.values(tripsUpdates).some(
            (t: any) =>
              (t.driverId === cleanDriverId ||
                (t.driverName &&
                  t.driverName.toLowerCase().trim() ===
      ...
      if (hasUpdates) {
        db.ref(
          systemContext === "Pg" ? "trips" : `${systemContext}/trips`,
        ).update(tripsUpdates);
      }
      ...
      data.trips.forEach((t: any) => {
        if (t.isTemp && t.status !== "Finalizada") {
          ...
          const fixedExists = data.trips.some(
            (ft: any) =>
              !ft.isTemp &&
              ft.status !== "Cancelada" &&
              ft.date === t.date &&
              ft.time === t.time &&
              (ft.driverId === t.driverId ||
                (ft.driverName &&
                  t.driverName &&
                  ft.driverName.toLowerCase().trim() ===
                    t.driverName.toLowerCase().trim())),
          );
          ...
          const activeSlot = activeSlots
            ...
            .sort((a, b) => {
              const diffA = Math.abs(
                timeToMinutes(addMinutes(a.time, 60)) - timeToMinutes(t.time),
              );
              const diffB = Math.abs(
                timeToMinutes(addMinutes(b.time, 60)) - timeToMinutes(t.time),
              );
              return diffA - diffB;
            })[0];
  ...
  const saveApiKey = (k: string) => {
    localStorage.setItem("nexflow_gemini_key", k);
    notify("API Key salva localmente (fallback)!", "success");
  };
  const blockIp = () => {
    if (!ipToBlock) return notify("Digite um IP", "error");
    dbOp("create", "blocked_ips", {
      ip: ipToBlock,
      reason: ipReason || "Manual",
      blockedBy: user.username,
    });
    setIpToBlock("");
    setIpReason("");
    notify("IP Bloqueado!", "delete");
  };
  const saveIpLabel = (ip: string, label: string) => {
    if (!ip) return;
  ...
  const del = (col: string, id: string) => {
    if (col === "passengers" && user?.role === "operator") {
      if (deletionCount >= 3) {
        return notify(
          "Limite de exclusão atingido (máx 3 por sessão). Contate a Coordenação.",
          "error",
        );
      }
    }
    ...
    if (col === "passengers") {
      requestConfirm(
        "Excluir Passageiro?",
        `Deseja excluir ${item?.name || "este passageiro"}?`,
        () => {
    ...
    if (col === "drivers") {
      requestConfirm(
        "Excluir Motorista?",
        `Deseja excluir ${item?.name || "este motorista"}?`,
        () => {
    ...
    if (col === "lostFound") {
      requestConfirm(
        "Excluir Item?",
        `Deseja excluir ${item?.item || "este item"}?`,
        () => {
    ...
    if (col === "trips") {
      ...
      if (trip) {
        ...
        requestConfirm("Excluir viagem?", msg, () => {
          logAction(
            "Excluiu Viagem",
            `Motorista: ${trip.driverName} - ${trip.date} ${trip.time}`,
          );
          triggerUndo(() => {
          ...
          if (trip.isMadrugada) {
            ...
            if (sp) {
              ...
              db.ref(
                tableSystemContext === "Pg"
                  ? `daily_tables/${trip.date}/madrugada/${sp.vaga}`
                  : `${tableSystemContext}/daily_tables/${trip.date}/madrugada/${sp.vaga}`,
              ).update({ time: null, qtd: null });
            }
          }
  ...
  const saveDriverName = (oldVaga: string) => {
    ...
    const newList = spList.map((d: any) =>
      d.vaga === oldVaga ? { ...d, name: tempName, vaga: tempVaga } : d,
    );
    const tableSystemContext =
      user.username === "Breno" && systemContext === "Mistura"
        ? "Pg"
    ...
    logAction(
      "Editou Tabela Geral",
      `Vaga ${oldVaga} -> ${tempVaga} (Motorista: ${tempName})`,
    );
    ...
    if (tempVaga !== oldVaga && tableStatus[oldVaga]) {
      ...
      const statusPath =
        tableSystemContext === "Pg"
          ? `daily_tables/${currentOpDate}/status`
          : `${tableSystemContext}/daily_tables/${currentOpDate}/status`;
    ...
    if (tableSystemContext === "Mip" || tableTab.startsWith("mip")) {
      ...
      if (oldVaga !== tempVaga) {
        db.ref(tripsPath)
          .child(`temp_${currentOpDate}_${mipDayType}_${oldVaga}_1`)
          .remove();
        db.ref(tripsPath)
          .child(`temp_${currentOpDate}_${mipDayType}_${oldVaga}_2`)
          .remove();
      }
            ...
            db.ref(tripsPath)
              ...
              .once("value", (snap) => {
                if (snap.exists()) {
                  ...
                  const driverDb = data.drivers.find(
                    (d: any) =>
  ...
  const updateMipDriver = (id: string, payload: any) => {
    ...
    const timeSuffix = tableTab === "mip18" ? "18" : "6";
    let node =
      tableSystemContext === "Mip"
        ? `Mip/drivers_${timeSuffix}_${mipDayType}`
        : tableSystemContext === "Pg"
          ? "drivers_table_list"
          : `${tableSystemContext}/drivers_table_list`;
    ...
    if (tableSystemContext === "Mip" || tableTab.startsWith("mip")) {
      ...
      if (driver) {
        ...
        const manageTempTrip = (
          ...
          const existingTrip = (data.trips || []).find(
            (t: any) =>
              t.isTemp &&
              t.date === currentOpDate &&
              t.vaga === vaga &&
          ...
          if (hasData) {
            if (existingTrip) {
              if (
                existingTrip.status !== "Finalizada" &&
                existingTrip.status !== "Cancelada"
              ) {
                db.ref(tripsPath)
                  .child(existingTrip.realId || existingTrip.id)
                  .update({
                    time: time || "",
                    tripNumber: num || "",
                    driverName: driverName,
              ...
              if (!fixedExists) {
                ...
                if (
                  suffix === "1" &&
                  (tableSystemContext === "Mip" || tableTab.startsWith("mip"))
                ) {
                  // Só aplica na primeira viagem para evitar confusão
                  const timeToMinutes = (t: string) => {
                    if (!t) return -1;
                    const [h, m] = t.split(":").map(Number);
                    return h * 60 + m;
                  ...
                  const candidates = (data.passengers || []).filter(
                    (p: any) => {
                      if (p.status !== "Ativo" || p.date !== currentOpDate)
                        return false;
                      const pTimeMins = timeToMinutes(p.time);
                      // Intervalo [tripTime, tripTime + 30]
                      return (
                        pTimeMins >= tripTimeMins &&
                        pTimeMins <= tripTimeMins + 30
                      );
                    },
                  );

                  if (candidates.length > 0) {
                    tripData.passengerIds = candidates.map(
                      (c: any) => c.realId || c.id,
                    );
                  }
                }
        ...
        if (
          payload.time1 !== undefined ||
          payload.num !== undefined ||
        ...
        if (payload.time2 !== undefined || payload.name !== undefined) {
          ...
          if (!isMip) {
            ...
            if (hasTime2) {
              ...
              const existingTrip2 = (data.trips || []).find(
                (t: any) =>
                  t.isTemp &&
                  t.date === currentOpDate &&
                  t.vaga === vaga &&
              ...
              if (
                existingTrip2 &&
                existingTrip2.status !== "Finalizada" &&
                existingTrip2.status !== "Cancelada"
              ) {
                db.ref(tripsPath)
                  .child(existingTrip2.realId || existingTrip2.id)
                  .remove();
              }
            }
          } else {
  ...
  const handleMipBaixar = (id: string) => {
    ...
    let sourceNode =
      tableTab === "mip18"
        ? `Mip/drivers_18_${mipDayType}`
        : `Mip/drivers_6_${mipDayType}`;
    ...
    if (driver.baixou) {
      ...
      const newList = spList.map((d: any) =>
        d.id === id ? { ...d, baixou: false, baixouAt: null } : d,
      );
      ...
      db.ref(targetNode).once("value", (snap) => {
        ...
        const indexToRemove = reverseList.findIndex(
          (d: any) => d.vaga === vaga && d.isCopy,
        );
        ...
        if (indexToRemove !== -1) {
          ...
          const fallbackIndex = reverseList.findIndex(
            (d: any) => d.vaga === vaga && !d.baixou && d.id !== driver.id,
          );
          if (fallbackIndex !== -1) {
    ...
    const newListWithBaixou = spList.map((d: any) =>
      d.id === id ? { ...d, baixou: true } : d,
    );
  ...
  const updateCannedMessage = (id: string, field: string, value: any) => {
    const newList = cannedMessages.map((m: any) =>
  ...
  const deleteCannedMessage = (id: string) => {
    requestConfirm(
      "Excluir mensagem?",
      "Esta mensagem será removida da lista.",
      () => {
  ...
  const saveExtraCharge = () => {
    if (
      !formData.value ||
      !formData.date ||
      (!formData.driverName && !formData.driverId)
    )
      return notify("Valor, Data e Motorista são obrigatórios.", "error");

    let driverName = formData.driverName;
    let phone = formData.phone;

    let dr = null;
    if (formData.driverId) {
      dr = data.drivers.find((d: any) => String(d.id) === String(formData.driverId));
    }
    if (!dr && formData.driverName) {
      dr = data.drivers.find(
        (d: any) =>
          d.name.trim().toLowerCase() === formData.driverName.trim().toLowerCase()
      );
    }

    if (dr) {
      driverName = dr.name;
      phone = dr.phone || (dr.phones && dr.phones[0]?.phone) || phone;
    }

    const passengers = suggestedTrip ? suggestedTrip.passengers : [];
    const passengerIds = passengers.map((p: any) => p.realId || p.id);
    const pCountSnapshot = passengers.reduce(
      (a: any, b: any) => a + parseInt(b.passengerCount || 1, 10),
      0,
    );

    // Garante ID Sequencial
    const nextId = formData.id ? formData.id : generateNextTripId();
    ...
    // Update passengers to 'Finalizada' just like regular trips
    if (passengers.length > 0) {
      passengers.forEach((p: any) => {
        if (p.status === "Ativo") {
          dbOp("update", "passengers", { id: p.id, status: "Finalizada" });
        }
      });
    }
  ...
  const autoAssignPassenger = async (pax: any) => {
    ...
    const isAssigned = data.trips.some(
      (t: any) =>
        t.date === pax.date &&
        t.status !== "Cancelada" &&
        (t.passengerIds || []).some(
          (pid: any) => String(pid) === String(pax.realId || pax.id),
        ),
    );
  ...
  const advanceQueue = () => {
    ...
    if (queue.length > 0 && index < queue.length - 1) {
      ...
      notify(
        `Passageiro ${nextIndex + 1}/${queue.length} carregado!`,
        "success",
      );
    ...
    try {
      if (collection === "passengers") {
        if (!formData.name || !formData.neighborhood)
          return notify("Nome e Bairro obrigatórios", "error");
        ...
        const blocked = data.passengers.find((p: any) => {
          ...
          const phoneSim =
            p.phone && formData.phone
              ? calculateSimilarity(p.phone, formData.phone)
              : 0;
        ...
        if (blocked) {
          return notify(
            `Passageiro BLOQUEADO encontrado: ${blocked.name}. Motivo: ${blocked.blockReason || "Não informado"}`,
            "error",
          );
        }
        ...
        const existing = data.passengers.find((p: any) => {
          ...
          const addressMatch =
            ...
            formData.address?.toLowerCase().trim();
        ...
        if (existing) {
          if (aiPassengerQueueRef.current.length > 0) {
            addPersistentNotification(
              `Passageiro pulado, ID ${existing.id} é idêntico para este dia e horário`,
            );
            advanceQueue();
            return;
          }
        ...
        await dbOp(formData.id ? "update" : "create", "drivers", {
          ...formData,
          phones,
        });
      } else if (collection === "lostFound") {
        if (!formData.description)
          return notify("Descrição obrigatória", "error");
        ...
        if (p && p.status === "Bloqueado") {
          return notify(
            `Passageiro BLOQUEADO: ${p.name}. Motivo: ${p.blockReason || "Não informado"}`,
            "error",
          );
        }
        ...
        if (!formData.sourceTime || !formData.newTime)
          return notify(
            "Preencha o horário de origem e o novo horário!",
            "error",
          );
        ...
        const passengersToReschedule = data.passengers.filter((p: any) => {
          ...
          const systemMatch =
            systemContext === "Mistura" || (p.system || "Pg") === systemContext;

          return (
            isSameTime && isSameDate && isNotBlocked && isNotBusy && systemMatch
          );
        });

        if (passengersToReschedule.length === 0)
          return notify(
            "Nenhum passageiro pendente encontrado para este horário!",
            "error",
          );
        ...
        notify(
          `${passengersToReschedule.length} passageiros reagendados!`,
          "success",
        );
        setModal(null);
      } else if (collection === "appointments") {
        if (!formData.passengerInput || !formData.date || !formData.time)
          return notify("Preencha todos os campos!", "error");
        const identifiers = formData.passengerInput
          .split(",")
          .map((s: string) => s.trim())
          .filter(Boolean);
        ...
        for (const idOrName of identifiers) {
          const normalizedId = idOrName
            .replace(/ /g, "")
            .replace("#", "_")
            .toUpperCase();
          const p = data.passengers.find(
            (p: any) =>
              String(p.id) === idOrName ||
              String(p.id).toUpperCase() === normalizedId ||
  ...
  const handleSmartCreate = async () => {
    ...
    if (!geminiKey)
      return notify(
        "Configure a API Key nas configurações para usar o Cadastro Mágico.",
        "error",
      );
    setAiLoading(true);
    try {
      ...
      if (!res)
        throw new Error("A IA não retornou nada. Verifique sua chave API.");
      ...
      const processedPassengers = jsonArray.map((json: any) => {
        ...
        if (json.payment) {
          const found = validPayments.find(
            (p) => p.toLowerCase() === json.payment.toLowerCase(),
          );
        ...
        if (!timeToUse) {
          const now = new Date();
          timeToUse = now.toLocaleTimeString("pt-BR", {
            hour: "2-digit",
            minute: "2-digit",
            timeZone: "America/Sao_Paulo",
          });
        ...
        const calculatedPassengerCount =
          (json.adultCount || 1) + countingChildren;
  ...
  const toggleMic = () => {
    ...
    recognition.onresult = (e: any) => {
      setAiInput(
        (prev: string) => (prev ? prev + " " : "") + e.results[0][0].transcript,
      );
      setIsListening(false);
    };
  ...
  const simulate = () => {
    // Relax check: Allow if driverId is present OR (driverName is present AND system is Mip) OR modal is extraCharge
    if (
      !formData.driverId &&
      !(
        (systemContext === "Mip" || modal === "extraCharge") &&
        formData.driverName
      )
    ) {
      // Tenta achar motorista pelo nome antes de falhar
      const drByName = data.drivers.find(
        (d: any) =>
          d.name.toLowerCase().trim() ===
          (formData.driverName || "").toLowerCase().trim(),
      );
      if (drByName) {
    ...
    let dr = data.drivers.find((d: any) => String(d.id) === String(formData.driverId));
    if (!dr && formData.driverId) {
      dr = data.drivers.find((d: any) => String(d.realId) === String(formData.driverId));
    }

    // If no driver found by ID (MIP case or extraCharge), construct a dummy driver object from name
    const driverObj =
      dr ||
      (formData.driverName
        ? { name: formData.driverName, capacity: 15, id: "temp" }
        : null);

    if (!driverObj) return notify("Motorista não identificado", "error");

    const driverCapacity = driverObj.capacity
      ? parseInt(driverObj.capacity, 10)
      : 15;
    ...
    if (candidates.length === 0)
      return notify(
        "Nenhum passageiro livre encontrado para este horário.",
        "info",
      );
    ...
    const anchor = [...candidates].sort((a: any, b: any) => {
      ...
      return (
        getBairroIdx(a.neighborhood, systemContext) -
        getBairroIdx(b.neighborhood, systemContext)
      ); // Menor índice geográfico
    })[0];
    ...
    candidates.sort((a: any, b: any) => {
      ...
      const sameAddrA =
        a.address &&
        anchor.address &&
        a.address.trim().toLowerCase() === anchor.address.trim().toLowerCase();
      const sameAddrB =
        b.address &&
        anchor.address &&
        b.address.trim().toLowerCase() === anchor.address.trim().toLowerCase();
    ...
    if (selectedPassengers.length < candidates.length) {
      notify(
        `Limite de ${driverCapacity} lugares atingido. Priorizando rota da maior família.`,
        "info",
      );
  ...
  useEffect(() => {
    if (modal === "extraCharge" && suggestedTrip && !editingTripId) {
      const paxCount = suggestedTrip.occupancy || 0;
      if (paxCount > 0 && data.extraChargeValue) {
        setFormData((prev: any) => ({
          ...prev,
          value: paxCount * data.extraChargeValue,
        }));
      } else if (data.extraChargeValue && !formData.value) {
        setFormData((prev: any) => ({ ...prev, value: data.extraChargeValue }));
      }
    }
  }, [suggestedTrip?.occupancy, modal, data.extraChargeValue]);
  ...
  const addById = () => {
    ...
    for (const idOrName of identifiers) {
      ...
      const currentCap = currentTrip.driver?.capacity
        ? parseInt(currentTrip.driver.capacity, 10)
        : 15;

      // Check overlap at same day
      const isOccupiedSameDay = data.trips.some(
        (t: any) =>
      ...
      if (isOccupiedSameDay) {
        notify(
          `Erro: ${p.name} já está em outra viagem nesta mesma data!`,
          "error",
        );
        continue;
      }
      ...
      if (currentTrip.occupancy + paxCount > currentCap) {
        notify(
          `Capacidade excedida para ${p.name}! Restam ${currentCap - currentTrip.occupancy} lugares.`,
          "error",
        );
        continue;
      }

      currentTrip.passengers = [...currentTrip.passengers, p].sort(
        (a, b) => getBairroIdx(a.neighborhood) - getBairroIdx(b.neighborhood),
      );
  ...
  const removePass = (pid: string) => {
    ...
    const newPassList = suggestedTrip.passengers.filter(
      (p: any) => p.id !== pid,
    );
    const newOcc =
      suggestedTrip.occupancy -
      parseInt(
        suggestedTrip.passengers.find((x: any) => x.id === pid)
          ?.passengerCount || 1,
      );
    setSuggestedTrip({
      ...suggestedTrip,
      passengers: newPassList,
      occupancy: newOcc,
    });
  };
  ...
  const confirmTrip = () => {
    ...
    const passengerIdsToSave = suggestedTrip.passengers.map(
      (p: any) => p.realId || p.id,
    );
    ...
    const sp = spList.find(
      (s: any) =>
        ...
        (suggestedTrip.driver.name || "").trim().toLowerCase(),
    );
    ...
    if (formData.responsibleUser && formData.responsibleUser !== "Sistema") {
      ...
      const existingTrip = data.trips.find(
        (tx: any) => tx.id === editingTripId,
      );
      if (
        existingTrip &&
        existingTrip.createdBy &&
        existingTrip.createdBy !== "Sistema"
      ) {
    ...
    if (formData.isMadrugada) {
      if (sp) {
        ...
        db.ref(
          tableSystemContext === "Pg"
            ? `daily_tables/${finalDate}/madrugada/${sp.vaga}`
            : `${tableSystemContext}/daily_tables/${finalDate}/madrugada/${sp.vaga}`,
        ).update({
          time: finalTime,
          qtd: pCount,
        });
      ...
      if (formData.isExtraMadrugada) {
        ...
        const path =
          tableSystemContext === "Pg"
            ? `daily_tables/${finalDate}/madrugadaFooter`
            : `${tableSystemContext}/daily_tables/${finalDate}/madrugadaFooter`;
    ...
    suggestedTrip.passengers.forEach((p: any) => {
      const pSystem = p.system || systemContext;
      const pId = p.realId || p.id;
      const passengerTime = (finalTime || "")
        .split("/")[0]
        .trim()
        .substring(0, 5);
      db.ref(
        pSystem === "Pg" ? `passengers/${pId}` : `${pSystem}/passengers/${pId}`,
      ).update({ time: passengerTime, date: finalDate });
    });
    ...
    if (!editingTripId) {
      logAction(
        "Criou Viagem",
        `Viagem #${tripId} (${isMadrugada ? "Madrugada" : "Normal"}) - Resp: ${payload.createdBy || user.username}`,
      );
    } else {
      logAction(
        "Editou Viagem",
        `Viagem #${tripId} - Resp: ${payload.createdBy || user.username}`,
      );
    }
  ...
  const openEditTrip = (t: any) => {
    if (t.isExtra) {
      ...
      if (t.passengersSnapshot && t.passengersSnapshot.length > 0) {
        ...
        pax = data.passengers.filter((p: any) =>
          (t.passengerIds || []).includes(p.realId || p.id),
        );
      }
      setSuggestedTrip({ passengers: pax });
      setModal("extraCharge");
      return;
    }
    let dr = data.drivers.find((d: any) => String(d.id) === String(t.driverId));
    ...
    if (t.passengersSnapshot && t.passengersSnapshot.length > 0) {
      pax = t.passengersSnapshot;
      occ = pax.reduce(
        (a: any, b: any) => a + parseInt(b.passengerCount || 1),
        0,
      );
    } else if (t.passengerIds && t.passengerIds.length > 0) {
      pax = data.passengers.filter((p: any) =>
      ...
      for (let i = 0; i < occ; i++)
        pax.push({
          id: `dummy_${i}`,
          name: "Passageiro Madrugada",
          neighborhood: "Madrugada",
          passengerCount: 1,
        });
    }
  ...
  const updateTripStatus = (id: string, status: string) => {
    ...
    if (trip && status === "Finalizada") {
      if (!trip.passengersSnapshot && trip.passengerIds) {
        const pax = data.passengers.filter((p: any) =>
          trip.passengerIds.includes(p.realId || p.id),
        );
        ...
        db.ref(
          tripSystem === "Pg"
            ? `trips/${realId}`
            : `${tripSystem}/trips/${realId}`,
        ).update({ passengersSnapshot: pax });
      }
    }
  };
  ...
  const duplicateTrip = (t: any) => {
    ...
    const newTrip = {
      ...
      isTemp: false,
      createdBy:
        !!t.isMadrugada && t.createdBy && t.createdBy !== "Sistema"
          ? t.createdBy
          : user.username,
      lastEditedBy: user.username,
    };
    ...
    Object.keys(newTrip).forEach(
      (key) => newTrip[key] === undefined && delete newTrip[key],
    );
  ...
  const updateTableStatus = (vaga: string, status: string | null) => {
    const tableSystemContext =
      user.username === "Breno" && systemContext === "Mistura"
        ...
        : systemContext;
    const newStatus = { ...tableStatus };
    if (status) newStatus[vaga] = status;
    else delete newStatus[vaga];
    ...
    db.ref(
      tableSystemContext === "Pg"
        ? `daily_tables/${currentOpDate}`
        : `${tableSystemContext}/daily_tables/${currentOpDate}`,
    ).update({ status: newStatus, lousaOrder: newLousa });
  };
  ...
  const toggleLousaFromConfirmados = (vaga: string) => {
    ...
    if (isInLousa) {
      ...
      const newOrder = [
        ...lousaOrder,
        { vaga, uid: generateUniqueId(), riscado: false },
      ];
      db.ref(
        tableSystemContext === "Pg"
          ? `daily_tables/${lousaDate}/lousaOrder`
  ...
  const cancelConfirmation = (vaga: string) => {
    const driverName =
      spList.find((d: any) => d.vaga === vaga)?.name || "Desconhecido";
    requestConfirm(
      "Cancelar Confirmação?",
      `Deseja realmente remover o motorista ${driverName} vaga ${vaga} da tabela Confirmados?`,
      () => {
        const tableSystemContext =
          user.username === "Breno" && systemContext === "Mistura"
            ? "Pg"
        ...
        db.ref(
          tableSystemContext === "Pg"
            ? `daily_tables/${currentOpDate}/status/${vaga}`
            : `${tableSystemContext}/daily_tables/${currentOpDate}/status/${vaga}`,
        ).remove();
  ...
  const handleLousaAction = (
    uid: string | null,
    action: string,
    vagaRef: string | null = null,
  ) => {
    const tableSystemContext =
      user.username === "Breno" && systemContext === "Mistura"
        ? "Pg"
    ...
    if (itemIndex === -1 && action !== "duplicate" && action !== "remove_all")
      return;
    ...
    if (action === "riscar") {
      ...
      logAction(
        newRiscadoState ? "Riscou Vaga na Lousa" : "Desriscou Vaga na Lousa",
        `Vaga: ${newLousa[itemIndex].vaga}`,
      );
    } else if (action === "remove") {
      const itemToRemove = newLousa[itemIndex];
      if (itemToRemove) {
        const driverName = itemToRemove.isNull
          ? "Vazio"
          : spList.find((d: any) => d.vaga === itemToRemove.vaga)?.name ||
            "Desconhecido";
        requestConfirm(
          "Remover da Lousa?",
          `Deseja realmente remover o motorista ${driverName} vaga ${itemToRemove.vaga} da tabela Lousa?`,
          () => {
      ...
      if (itemIndex > -1) {
        ...
        const lastIndex = [...newLousa]
          .reverse()
          .findIndex((d: any) => d.vaga === vaga);
        if (lastIndex !== -1) {
  ...
  const removeMadrugadaVaga = (
    vaga: string,
    isExtraMadrugada: boolean = false,
  ) => {
    if (isExtraMadrugada) {
      ...
      const trip = data.trips.find(
        (t: any) =>
          t.isExtraMadrugada &&
          (t.vaga === vaga || t.driverName === vaga) &&
          t.status !== "Cancelada",
      );
      if (trip) {
        requestConfirm(
          "Remover Extra da Madrugada?",
          `Deseja realmente remover o motorista extra ${trip.driverName}? A viagem dele será cancelada.`,
          () => {
            dbOp("update", "trips", { id: trip.id, status: "Cancelada" });
            logAction(
              "Removeu Extra da Madrugada",
    ...
    const driverName =
      spList.find((d: any) => d.vaga === vaga)?.name || "Desconhecido";
    requestConfirm(
      "Remover esta vaga da madrugada?",
      `Deseja realmente remover o motorista ${driverName} vaga ${vaga} da tabela Madrugada?`,
      () => {
  ...
  const toggleMadrugadaRiscado = (vaga: string) => {
    ...
    if (currentData.riscado) {
      db.ref(
        tableSystemContext === "Pg"
          ? `daily_tables/${currentOpDate}/madrugada/${vaga}`
          : `${tableSystemContext}/daily_tables/${currentOpDate}/madrugada/${vaga}`,
      ).update({ riscado: false, comment: null });
  ...
  const confirmMadrugadaBlock = () => {
    ...
    db.ref(
      tableSystemContext === "Pg"
        ? `daily_tables/${currentOpDate}/madrugada/${vagaToBlock}`
        : `${tableSystemContext}/daily_tables/${currentOpDate}/madrugada/${vagaToBlock}`,
    ).update({
      riscado: true,
      comment: tempJustification,
    });
    logAction(
      "Riscou Vaga na Madrugada",
      `Vaga: ${vagaToBlock} - Motivo: ${tempJustification}`,
    );
    setModal(null);
    setVagaToBlock(null);
  };
  ...
  const openMadrugadaTrip = (vaga: string, date: string) => {
    ...
    const extraTrip = data.trips.find(
      (t: any) =>
        t.isExtraMadrugada &&
        t.date === date &&
  ...
  const sendBillingMessage = (trip: any) => {
    const currentUserData = (data.users || []).find(
      (u: any) => u.id === user.uid || u.username === user.username,
    );
    const pixInfo =
      currentUserData?.pixName && currentUserData?.pixKey
        ? `\n\n*Dados para Pagamento (Pix)*\nNome: ${currentUserData.pixName}\nChave: ${currentUserData.pixKey}`
        : "";

    // Procura o motorista para pegar o telefone mais atualizado
    let d = data.drivers.find((x: any) => String(x.id) === String(trip.driverId));
    
    // Fallback: se não achar por ID, tenta por nome (para viagens velhas ou manuais sem ID)
    if (!d && trip.driverName) {
      d = data.drivers.find(
        (x: any) =>
          x.name.trim().toLowerCase() === trip.driverName.trim().toLowerCase()
      );
    }
    ...
    if (d) {
      driverName = d.name;
      phones = d.phones && d.phones.length > 0 
        ? d.phones 
        : (d.phone ? [{ name: d.name, phone: d.phone }] : []);
    } else if (trip.isExtra && trip.extraPhone) {
      // Caso seja extra e não tenha motorista cadastrado, usa o telefone salvo na viagem
      phones = [{ name: trip.driverName, phone: trip.extraPhone }];
    }
    ...
    let msg = "";
    if (trip.isExtra) {
      msg = `Olá ${driverName}, referente à viagem do dia ${formatDisplayDate(trip.date)} às ${trip.time}. Valor: R$ ${Number(trip.value).toFixed(2).replace(".", ",")}. Status: ${trip.isPaid ? "PAGO" : "PENDENTE"}.${pixInfo}`;
    } else {
      msg = `Olá ${driverName}, referente à viagem #${trip.id} do dia ${formatDisplayDate(trip.date)} às ${trip.time}. Valor: R$ ${Number(trip.value).toFixed(2).replace(".", ",")}. Status: ${trip.isPaid ? "PAGO" : "PENDENTE"}.${pixInfo}`;
    }

    if (phones.length === 1) {
      window.open(
        `https://wa.me/55${phones[0].phone.replace(/\D/g, "")}?text=${encodeURIComponent(msg)}`,
        "_blank",
      );
    } else {
      setFormData({
  ...
  const sendPranchetaBillingMessage = (
    vaga: string,
    driverName: string,
    phone: string,
...
Agradecemos pela atenção e desejamos um bom trabalho a todos!${pixInfo}`;
    ...
    if (_phones.length === 1) {
      window.open(
        `https://wa.me/55${_phones[0].phone.replace(/\D/g, "")}?text=${encodedMsg}`,
        "_blank",
      );
    } else {
      setFormData({
        ...
        onSelect: (selectedPhone: string) => {
          window.open(
            `https://wa.me/55${selectedPhone.replace(/\D/g, "")}?text=${encodedMsg}`,
            "_blank",
          );
        },
      });
  ...
  const sendPranchetaBlockedMessage = (
    vaga: string,
    driverName: string,
    phone: string,
    phones?: Array<{ name: string; phone: string }>,
    ...
    const formattedPranchetaValue = pranchetaValue.toLocaleString("pt-BR", {
  ...
  if (isLoading)
    return (
      ...
      >
        <div className="text-amber-500 font-bold">CARREGANDO...</div>
      </div>
    );
  ...
  return (
    <SubscriptionLock user={user} systemContext={systemContext}>
      <div
        className={`h-[100dvh] w-full overflow-hidden ${theme.bg} ${theme.text} font-sans flex`}
        onTouchStart={handleGlobalTouchStart}
        onTouchEnd={handleGlobalTouchEnd}
      >
        {user && user.username === "Breno" && (
          <div
            className={`fixed top-2 left-1/2 -translate-x-1/2 z-50 bg-gray-800/90 backdrop-blur-md p-1 rounded-full flex items-center gap-1 border border-white/10 shadow-xl transition-all duration-300 ${isSystemSelectorExpanded ? "w-auto px-2" : "w-10 h-10 justify-center"}`}
          >
            {isSystemSelectorExpanded ? (
              <>
                {["Mistura", "Pg", "Mip", "Sv"]
                  .filter((sys) => sys !== "Mistura")
                  .map((sys) => {
        ...
        <AdminAuthModal
          ...
          onAuth={() => {
            ...
            notify(
              "Autorizado com sucesso! Agora você pode apagar múltiplos dados.",
              "success",
            );
          }}
          theme={theme}
        ...
        <div
          className={`flex-1 flex flex-col h-full min-w-0 ${theme.contentBg || "bg-black/20"}`}
        >
          {/* Header */}
          <div
            className={`h-16 flex items-center justify-between px-4 md:px-8 border-b ${theme.border} bg-opacity-80 backdrop-blur-md z-30 flex-shrink-0`}
          >
            <div className="flex items-center gap-4 flex-1">
              ...
              <h2
                className={`font-bold text-lg md:text-xl truncate ${["passengers", "drivers", "trips", "achados", "lostFound"].includes(view) && searchTerm ? "hidden md:block" : "block"}`}
              >
                {orderedMenuItems.find((i) => i.id === view)?.l ||
                  "Bora de Van"}
              </h2>
              {[
                "passengers",
                ...
                <div className="flex-1 max-w-md ml-auto md:ml-4 relative">
                  <div className="relative group">
                    ...
                    <input
                      ...
                      onFocus={() =>
                        searchTerm && setShowSearchSuggestions(true)
                      }
                      onBlur={() =>
                  ...
                  <AnimatePresence>
                    {showSearchSuggestions &&
                      [
                        ...
                        >
                          <div className="px-3 py-2 text-[10px] font-black uppercase tracking-widest opacity-40">
                            Filtrar por:
                          </div>
                          <button
                            onClick={() => {
            ...
            <div className="flex gap-2 ml-2">
              ...
              <button
                ...
                className={`p-2.5 rounded-xl ${theme.ghost || "bg-white/5 hover:bg-white/10 text-white/50"} hidden md:flex items-center gap-2 text-xs font-bold border ${theme.divider || "border-white/5"} mr-2`}
              ...
              {view !== "lostFound" &&
                ...
                view !== "billing" && (
                  <button
                    onClick={() =>
                      setFilterStatus(
                        filterStatus === "Ativo" ? "Todos" : "Ativo",
                      )
                    }
                    className={`p-2 rounded-lg ${filterStatus === "Ativo" ? theme.accent : "opacity-50"}`}
                  >
              ...
              <button
                onClick={() => {
                  if (view === "passengers") {
                    ...
                    setFormData({
                      neighborhood:
                        systemContext === "Mip" ? BAIRROS_MIP[0] : BAIRROS[0],
                      status: "Ativo",
                      payment: "Dinheiro",
                      passengerCount: 1,
                      luggageCount: 0,
                    ...
                    nextHalfHour.setMinutes(
                      Math.ceil(now.getMinutes() / 30) * 30,
                    );
                    nextHalfHour.setSeconds(0);

                    const dateStr = getTodayDate();
                    const timeStr = nextHalfHour.toLocaleTimeString("pt-BR", {
                      hour: "2-digit",
                      minute: "2-digit",
                    });
                    ...
                    setFormData({
                      ...
                      value: data.extraChargeValue || 0,
                      responsibleUser: user.username,
                    });
                    setSuggestedTrip({ passengers: [] });
                    setModal("extraCharge");
                  } else if (view === "lostFound") {
                    setFormData({ date: getTodayDate(), status: "Pendente" });
                    setModal("lostFound");
                ...
                className={`${theme.primary} p-2.5 rounded-xl shadow-lg active:scale-95`}
              >
                <Icons.Plus />
              </button>
            </div>
            ...
            <div className="max-w-6xl mx-auto pb-20">
              ...
              {view === "folgasGanchos" && (
                <FolgasGanchos
                  ...
                  systemContext={systemContext}
                  user={user}
                  folgasDisabled={folgasDisabled}
                  saturdayFolgaDisabled={saturdayFolgaDisabled}
                  customDefaultFolgas={customDefaultFolgas}
                  saturdayRotation={saturdayRotation}
                  tableWeekId={tableWeekId}
                />
              )}
              ...
              {view === "table" && (
                <Tabela
                  ...
                  mipDayType={mipDayType}
                  setMipDayType={setMipDayType}
                  currentOpDate={currentOpDate}
                  ...
                  setTempVaga={setTempVaga}
                  saveDriverName={saveDriverName}
                  updateTableStatus={updateTableStatus}
                  currentRotatedList={getRotatedList(currentOpDate)}
                  confirmedTimes={confirmedTimes}
                  isTimeExpired={isTimeExpired}
                  lousaOrder={lousaOrder}
                  toggleLousaFromConfirmados={toggleLousaFromConfirmados}
                  cancelConfirmation={cancelConfirmation}
                  handleLousaAction={handleLousaAction}
                  startLousaTime={startLousaTime}
                  addMadrugadaVaga={addMadrugadaVaga}
                  madrugadaList={madrugadaList}
                  removeMadrugadaVaga={removeMadrugadaVaga}
                  toggleMadrugadaRiscado={toggleMadrugadaRiscado}
                  duplicateMadrugadaDriver={duplicateMadrugadaDriver}
                  spList={spList}
                  madrugadaData={madrugadaData}
                  openMadrugadaTrip={openMadrugadaTrip}
                  openExtraMadrugadaTrip={openExtraMadrugadaTrip}
                  cannedMessages={cannedMessages}
                  addCannedMessage={addCannedMessage}
                  updateCannedMessage={updateCannedMessage}
                  deleteCannedMessage={deleteCannedMessage}
                  addNullLousaItem={addNullLousaItem}
                  addNullMadrugadaItem={addNullMadrugadaItem}
                  notify={notify}
                  requestConfirm={requestConfirm}
                ...
                <Financeiro
                  ...
                  togglePaymentStatus={(trip: any) => {
                    ...
                    if (
                      ...
                      const displayReceiver =
                        trip.receivedBy === "Breno"
                          ...
                          : trip.receivedBy;
                      return notify(
                        `Apenas ${displayReceiver} ou Coordenação pode desfazer este pagamento.`,
                        "error",
                      );
                    }
                    ...
                    if (isPaying) {
                      ...
                      logAction(
                        "Pagamento Desfeito",
                        `Passageiro: ${trip.passengerName} (Viagem #${trip.id})`,
                      );
                    }
                  }}
              ...
              {view === "settings" && (
                <Configuracoes
                  ...
                  saveDeviceLabel={(deviceId: string, label: string) =>
                    dbOp("update", `device_labels`, { [deviceId]: label })
                  }
        ...
        {paymentConfirmTrip && (
          <div
            className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/40 backdrop-blur-[2px] p-4 animate-in fade-in duration-200"
            onClick={() => setPaymentConfirmTrip(null)}
          >
            <div
            ...
            >
              <div className="flex flex-col items-center gap-4">
                ...
                <div className="text-center">
                  <h3
                    className={`font-black text-sm uppercase tracking-widest ${theme.text}`}
                  >
                    Data do Recebimento
                  </h3>
                  <p className="text-[10px] opacity-40 mt-1 uppercase font-bold">
                    Viagem #{paymentConfirmTrip.id}
                  </p>
                </div>
              </div>
              ...
              <button
                onClick={() => {
                  ...
                  logAction(
                    "Pagamento Recebido",
                    `Viagem #${paymentConfirmTrip.id} (${paymentConfirmTrip.driverName || "Passageiro"}) - Data Ref: ${paymentConfirmDate}`,
                  );
        ...
        {activeReminder && (
          <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[9999] w-full max-w-sm px-4 animate-bounce-in">
            <div
              className={`${theme.card} border-2 border-amber-500 shadow-[0_0_50px_rgba(245,158,11,0.5)] p-6 rounded-[2.5rem] flex flex-col gap-5 relative overflow-hidden backdrop-blur-2xl animate-pulse-gentle`}
            >
              ...
              <div className="flex items-start gap-5 relative z-10">
                ...
                <div className="flex-1 min-w-0 pt-1">
                  <h4 className="font-black text-amber-500 text-[10px] uppercase tracking-[0.3em] mb-2">
                    Atenção: Lembrete
                  </h4>
        ...
        {activeSiteNotification && (
          <div className="fixed bottom-6 left-4 right-4 md:left-auto md:right-6 z-[9999] w-auto md:w-full md:max-w-sm animate-bounce-in">
            <div
              className={`${theme.card} border-2 ${theme.border} shadow-2xl p-5 md:p-6 rounded-[2rem] md:rounded-[2.5rem] flex flex-col gap-4 md:gap-5 relative overflow-hidden backdrop-blur-2xl animate-pulse-gentle`}
            >
              <div
                className={`absolute inset-0 bg-gradient-to-br ${themeKey === "solar" ? "from-amber-500/10" : "from-white/5"} to-transparent animate-pulse`}
              ></div>
              <div className="flex items-start gap-4 md:gap-5 relative z-10">
                <div
                  className={`${theme.primary} p-3 md:p-4 rounded-2xl md:rounded-3xl shadow-xl animate-bounce`}
                >
                  <Icons.Calendar size={24} className="md:w-7 md:h-7" />
                </div>
                <div className="flex-1 min-w-0 pt-1">
                  <h4
                    className={`font-black ${theme.accent} text-[9px] md:text-[10px] uppercase tracking-[0.3em] mb-1 md:mb-2`}
                  >
                    Agendamento via Site
                  </h4>
                  <p
                    className={`text-base md:text-lg font-black leading-tight ${theme.text} drop-shadow-sm`}
                  >
                    {activeSiteNotification.passengerName} acabou de agendar!
                  </p>
                  <div className="mt-1.5 md:mt-2 flex items-center gap-2">
                    <span
                      className={`px-2 py-0.5 ${theme.inner} rounded-full text-[9px] md:text-[10px] font-bold uppercase tracking-wider`}
                    >
        ...
        {showSnoozeModal && (
          <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in">
            <div
              className={`${theme.card} border ${theme.border} w-full max-w-xs p-8 rounded-[2.5rem] shadow-2xl stagger-in`}
            >
              <div className="flex flex-col items-center text-center mb-8">
                ...
                <p className="text-xs opacity-50 mt-1">
                  Escolha um novo horário para o alerta
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <button
                  ...
                  className="py-4 rounded-2xl font-bold text-sm bg-white/5 hover:bg-white/10 transition-all"
                >
                  Cancelar
                </button>
                <button
                  onClick={handleSnooze}
                  className={`${theme.primary} py-4 rounded-2xl font-black text-sm shadow-xl shadow-primary/20 active:scale-95 transition-all`}
                >
                  Confirmar
                </button>
              </div>
        ...
        <GlobalModals
          ...
          aiPassengerQueue={aiPassengerQueue}
          aiPassengerIndex={aiPassengerIndex}
          isListening={isListening}
          toggleMic={toggleMic}
          handleSmartCreate={handleSmartCreate}
          aiLoading={aiLoading}
          theme={theme}
          themeKey={themeKey}
          formData={formData}
          ...
          spList={spList}
          madrugadaList={madrugadaList}
          tempVagaMadrugada={tempVagaMadrugada}
          setTempVagaMadrugada={setTempVagaMadrugada}
          confirmAddMadrugadaVaga={confirmAddMadrugadaVaga}
          vagaToBlock={vagaToBlock}
          tempJustification={tempJustification}
          setTempJustification={setTempJustification}
          confirmMadrugadaBlock={confirmMadrugadaBlock}
          showNewsModal={showNewsModal}
          latestNews={latestNews}
          markNewsAsSeen={markNewsAsSeen}
          systemContext={systemContext}
          user={user}
        />
